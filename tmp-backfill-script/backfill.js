...
function inLookbackWindow(value) {
...
async function fetchSocrataRecords(url) {
...
function computeConfidence(tabcName, tabcAddress, ctrlName, ctrlAddress) {
    const nameDist = editDistance(normalizeName(tabcName), normalizeName(ctrlName));
    const addrDist = editDistance(tabcAddress.toLowerCase().trim(), ctrlAddress.toLowerCase().trim());
...
async function phase1_tabcIngest() {
    ...
    log(`Phase 1: Starting TABC ingest (county=${COUNTY_FILTER || "all"}, lookback=${LOOKBACK_MONTHS} months)…`);
    let totalWritten = 0;
    // Issued licenses
    ...
    for (let offset = 0;; offset += PAGE_SIZE) {
        ...
        for (const r of records) {
            if (!r.license_id)
                continue;
            if (!inLookbackWindow(r.current_issued_date))
                continue;
    ...
    for (let offset = 0;; offset += PAGE_SIZE) {
        ...
        for (const r of records) {
            if (!r.applicationid)
                continue;
            if (!inLookbackWindow(r.submission_date))
                continue;
    ...
    await backfillRef.set({
        ...
        tabc_completedAt: firestore_1.FieldValue.serverTimestamp(),
    }, { merge: true });
...
async function buildAddressIndex() {
    ...
    for (const doc of snap.docs) {
        ...
        if (COUNTY_FILTER && estCounty !== COUNTY_FILTER)
            continue;
...
async function phase2_comptrollerBackfill() {
    ...
    log(`Phase 2: Building establishment address index (county=${COUNTY_FILTER || "all"}, lookback=${LOOKBACK_MONTHS} months)…`);
    ...
    for (const month of months) {
        ...
        for (let offset = 0;; offset += PAGE_SIZE) {
            const where = encodeURIComponent(`obligation_end_date_yyyymmdd >= '${monthStart}' AND obligation_end_date_yyyymmdd < '${nextMonthStart}'`);
            ...
            for (const r of records) {
                ...
                if (addrMatch) {
                    confidence = computeConfidence(addrMatch.name, addrMatch.address, r.taxpayer_name ?? r.location_name ?? "", r.location_address ?? "");
                ...
                if (matchId) {
                    ...
                    batch.set(ref, {
                        ...
                        "comptroller.monthlyRecords": firestore_1.FieldValue.arrayUnion(monthRecord),
        ...
        monthsProcessed++;
        log(`  Phase 2: ${month} done — fetched=${monthFetched}, considered=${monthConsidered}, matched=${monthMatched}, unmatched=${monthUnmatched}`);
    }
    await backfillRef.set({
        ...
        comptroller_completedAt: firestore_1.FieldValue.serverTimestamp(),
    }, { merge: true });
