...
import { devLogin } from "@/components/shared/AuthProvider";
...
export default function LoginPage() {
  ...
  const handleDevLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (devUser === "admin" && devPass === "admin") {
      devLogin();
    ...
    <section className="min-h-[80vh] flex flex-col items-center justify-center gap-6">
      <div className="bg-white rounded-2xl shadow-lg p-10 flex flex-col items-center gap-4 border border-gray-100 max-w-sm w-full">
        ...
        <div className="w-full border-t border-gray-100 pt-4 mt-2">
          <p className="text-xs text-gray-400 text-center mb-3">Dev access</p>
          <form onSubmit={handleDevLogin} className="flex flex-col gap-2">
            ...
            <button
              type="submit"
              className="w-full bg-[#1a2233] text-white font-medium px-6 py-2 rounded-lg text-sm hover:bg-[#2a3447] transition"
            >
              Dev Login
            </button>
          </form>
